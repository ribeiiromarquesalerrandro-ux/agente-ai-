...
const VIEW_IDS: View[] = ["chat", "tools", "config", "desktop", "github", "access"];

function initialView(): View {
  if (typeof window === "undefined") return "chat";
  const requested = new URLSearchParams(window.location.search).get("view");
  return requested && VIEW_IDS.includes(requested as View) ? requested as View : "chat";
}
...
function ConfigPanel({ draft, setDraft, models, allowedModels, onConnect, onSave, saving, isOwner, isProMax }: { draft: any; setDraft: (value: any) => void; models: string[]; allowedModels: string[]; connecting: boolean; onConnect: () => void; onSave: () => void; saving: boolean; isOwner: boolean; isProMax: boolean }) {
  ...
  return <div className="agent-workbench mx-auto max-w-5xl p-4 sm:p-8"><PanelHeading code="AGENT_CONFIG" title="NÚCLEO DO AGENTE" description={isProMax ? "Pro Max habilita a configuração ampliada de contexto e o acesso aos recursos liberados pelo owner." : "O plano Básico permite chat local e configuração essencial. O owner pode atribuir Pro Max para liberar recursos ampliados."} /><div className="system-card agent-model-strip mb-5 p-4"><p className="font-mono text-[10px] tracking-[0.14em] text-cyan-300">[MODELOS AUTORIZADOS PARA ESTE PLANO]</p><div className="mt-3 flex flex-wrap gap-2">{allowedModels.map((model) => <Badge key={model} className="system-badge">{model}</Badge>)}</div></div>
...
function DesktopPanel() { const platforms = [{ name: "Windows", command: "Instale o aplicativo Electron ou use o navegador Chromium/Edge e selecione “Instalar aplicativo”.", code: "winget install Ollama.Ollama" }, { name: "macOS", command: "Instale o aplicativo Electron ou abra no Safari/Chrome e adicione o aplicativo à área de trabalho.", code: "brew install --cask ollama" }, { name: "Linux", command: "Use o pacote Electron compatível com sua distribuição ou instale pelo navegador como PWA.", code: "curl -fsSL https://ollama.com/install.sh | sh" }]; return <div className="desktop-stage mx-auto max-w-5xl p-4 sm:p-8"><PanelHeading code="DESKTOP_INSTALL" title="NÓ LOCAL MULTIPLATAFORMA" description="O modo desktop compartilha a mesma interface da web e usa o Ollama instalado na própria máquina. A aplicação pode funcionar como Electron ou PWA instalável." /><div className="grid gap-5 md:grid-cols-3">{platforms.map((platform) => <section className="platform-card system-card p-5" key={platform.name}><Download className="size-6 text-fuchsia-300" /><h3 className="mt-4 font-mono text-lg text-cyan-100">{platform.name}</h3><p className="mt-3 text-sm leading-6 text-zinc-400">{platform.command}</p><pre className="mt-5 overflow-x-auto border border-white/10 bg-black/40 p-3 font-mono text-xs text-cyan-200">{platform.code}</pre></section>)}</div><section className="install-sequence system-card mt-5 p-5"><p className="font-mono text-xs tracking-[0.16em] text-cyan-300">[INITIAL_SEQUENCE]</p><ol className="mt-4 space-y-3 text-sm leading-6 text-zinc-300"><li>01. Instale o Ollama no sistema operacional escolhido.</li><li>02. Baixe ao menos um modelo, por exemplo: <code className="text-fuchsia-200">ollama run llama3.2</code>.</li><li>03. Abra o aplicativo e mantenha a URL padrão <code className="text-fuchsia-200">http://localhost:11434</code> nas configurações.</li><li>04. Verifique a conexão e selecione o modelo local disponível.</li></ol></section></div>; }
